...
label = "batu"
subject = "han"
movements=["pinch","power","point","open"]
duration=5
prep_duration=2
print(os.getcwd())
folderName = os.getcwd()+"\\Dataset\\"
filename = folderName+label + "-" + subject + "-" + startTime + ".csv"
    ...
    def on_frame(self, controller):
        ...
        if move != 0:
            ...
            if not (frame.hands.is_empty and frame.gestures().is_empty):
                ...
                global visualize
                visualize = [[right_palm_position_x, right_palm_position_y, right_palm_normal_z],
                             [right_wrist_position_x, right_wrist_position_y, right_wrist_position_z],
                             [right_elbow_position_x, right_elbow_position_y, right_elbow_position_z],
                             [right_thumb_distal_end_x, right_thumb_distal_end_y, right_thumb_distal_direction_z],
                             [right_index_distal_end_x, right_index_distal_end_y, right_index_distal_end_z],
                             [right_middle_distal_end_x, right_middle_distal_end_y, right_middle_distal_end_z],
                             [right_ring_distal_end_x, right_ring_distal_end_y, right_ring_distal_direction_z],
                             [right_pinky_distal_end_x, right_pinky_distal_end_y, right_pinky_distal_end_z]]
                line = ""
                ...
                ax.scatter(visualize[0][0], visualize[0][1], visualize[0][2], c='r', s=100)
                plt.draw()
                print visualize[0][0], visualize[0][1], visualize[0][2]
...
def movement_change(duration):
    global move
    global move_next
    count=0
    move = 0
    print "Prepare for", movements[0]
    time.sleep(prep_duration)
    while True:
        move = movements[count % len(movements)]
        move_next = movements[(count+1) % len(movements)]
        time.sleep(duration) # sleep for duration
        move = 0
        print "Prepare for", move_next
        time.sleep(prep_duration)
        count=count+1
...
def main():
    ...
    t = threading.Thread(target=movement_change(duration)) # start a child thread
    t.daemon = True
    t.start()
    plt.ion()
